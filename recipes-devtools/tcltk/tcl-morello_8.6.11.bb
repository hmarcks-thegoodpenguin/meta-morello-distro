inherit autotools ptest binconfig purecap-sysroot

MORELLO_SRC = "poky/meta/recipes-devtools/tcltk/tcl_8.6.11.bb"

SUMMARY = "Tool Command Language  - CHERI: sourced from poky/meta"
...
FILESEXTRAPATHS:prepend := "${THISDIR}/tcl:${THISDIR}/cheri-patches:"
...
DEPENDS = "tcl-native zlib-morello"

PVTCL = "8.6.11"
BPNTCL = "tcl"

BASE_SRC_URI = "${SOURCEFORGE_MIRROR}/tcl/tcl-core${PVTCL}-src.tar.gz \
...
SRC_URI += "file://0001-tclCompCmdsSZ-cast-to-uintptr_t.patch"
...
do_install() {
	...
	for dir in compat generic unix; do
		install -d ${D}${includedir}/${BPNTCL}${VER}/$dir
		install -m 0644 ${S}/$dir/*.h ${D}${includedir}/${BPNTCL}${VER}/$dir/
	done
}

SYSROOT_DIRS += "${bindir_crossscripts}"

PACKAGES =+ "tcl-lib-morello"
FILES:tcl-lib-morello = "${libdir}/libtcl8.6.so.*"
FILES:${PN} += "${libdir}/tcl${VER} ${libdir}/tcl8.6 ${libdir}/tcl8"
FILES:${PN}-dev += "${libdir}/tclConfig.sh ${libdir}/tclooConfig.sh"

# isn't getting picked up by shlibs code
RDEPENDS:${PN} += "tcl-lib-morello"
